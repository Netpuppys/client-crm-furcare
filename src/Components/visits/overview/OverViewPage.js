...
const buttons = [ 'All Visits', 'Inpatient Visits', 'Outpatient Visits', 'Pinned Visits' ]
    ...
    <div className='w-full h-[calc(100vh-4.75rem)] px-[36px] py-4 overflow-y-auto relative'>
        ...
        <div className='w-full border-b border-[#CACDD8] mt-6 max-w-[44.5rem]'>
            ...
            <button
                ...
                className={`rounded-t-lg relative ${item===activeButton? "border-t-2 -bottom-[2px] text-[#0263E0] bg-white " : ""} border-[#0263E0] h-10 bg-white overflow-hidden`}
            >
                <p className={`w-full h-full flex items-center justify-center ${item===activeButton? "border-x" : ""} border-[#CACDD8] capitalize px-4`}>
                    {item}
                </p>
            </button>))}
        </div>
