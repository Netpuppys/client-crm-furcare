...
const ForgotPassword = ({ setForgotPassword }) => {
  ...
  const sentmessage = (
    <>
      A temporary password has been sent to your mobile {maskedMobileNumber}.
      Try logging in again using the new password.
    </>
  );
  ...
  return (
    <div className="max-w-[27rem] w-full flex flex-col items-center justify-start">
      ...
      <p className="text-center text-[#121C2D] text-sm leading-[1.25rem]">
        {codeSent
          ? sentmessage
          : "Enter you registered mobile number to receive a temporary password to login."}
      </p>
      {!codeSent && (
        <div className="max-w-80 w-full flex flex-col items-center justify-start">
          ...
          <div className="flex gap-5">
            <button
              ...
              className="px-4 py-2 text-text-black bg-transparent border-2 border-[#CACDD8] rounded-md font-semibold leading-[1.25rem] text-sm"
            >
              Back to login
            </button>
            <BlueButton
      ...
      {codeSent && (
        <button
          onClick={() => setForgotPassword(false)}
          className="px-4 py-2 mt-6 text-text-black bg-transparent border-2 border-[#CACDD8] rounded-md font-semibold leading-[1.25rem] text-sm"
        >
          Back to login
        </button>
      )}
