    ...
    <div className="w-full min-h-full px-[36px] py-4 ">
      <div className="flex items-start justify-between">
        <div className="text-[#0263E0] text-xs">
          ...
          <span> / </span>
          <p className="underline inline cursor-default">Settings</p>
        </div>
      </div>
          ...
          {buttonsArray.map((item, index) => (
            <button
              ...
              className={`rounded-t-lg relative ${
                item.serverName === activeButton
                  ? "border-t-2 -bottom-[2px] bg-white "
                  : ""
              } border-[#0263E0] h-10 bg-white overflow-hidden`}
            >
              <p
                className={`w-full h-full flex items-center font-medium justify-center ${
                  item.serverName === activeButton
                    ? "border-x text-[#0263E0]"
                    : "text-[#606B85]"
                } border-[#CACDD8] px-4`}
              >
                {item.name}
      ...
      <div
        className={`w-full mt-8 transition-all duration-200 border border-[#D9D9D9] rounded-md overflow-hiden ${
      ...
      {/* Department form */}
      <div
        className={`w-full mt-4 transition-all duration-200 border border-[#D9D9D9] rounded-md overflow-hiden ${
