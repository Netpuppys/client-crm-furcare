...
const EditLead = ({ selectedLead }) => {
  const [active, setActive] = useState(false);
  const [showDropdown, setShowDropdown] = useState(false);
  const [formData, setFormData] = useState({
    description: "", // Set the initial state of the description field
  });
  ...
  return (
    <div className="w-full h-[100%] relative overflow-y-auto px-[2.8rem] pt-[1.6rem]">
      ...
      <div className="mt-[1.75rem]">
        ...
        <div
          className={`mt-1 w-[calc(100%+2px)] h-[calc(2.25rem+2px)] border relative rounded-md border-[#8891AA] bg-white`}
        >
          <div
          ...
          >
            <div className="px-2 flex items-center justify-start gap-1 h-full py-1">
              <p className={`text-sm font-medium text-[#121C2D]`}>Select</p>
            </div>
      ...
      <div className="mt-[1.75rem]">
        ...
        />
        <p className="text-[#606B85] text-sm">Max 50 chars</p>
      </div>

      <div className="mt-12 border-b border-[#545456] border-opacity-35 pb-1">
        <p className="text-[#3C3C43] text-opacity-60 font-semibold ">Sign in</p>
      </div>

      <div className="flex flex-col gap-4 w-fit mt-[1.6rem] pl-2">
        <button className="px-4 py-2 w-fit disabled:bg-[#E1E3EA] disabled:border-[#E1E3EA] disabled:text-white hover:bg-transparent hover:text-[#C72323] border border-[#C72323] text-white text-nowrap bg-[#C72323] rounded-md font-medium leading-[1.25rem] text-sm transition-all">
          Facebook
        </button>
        <button className="px-4 py-2 w-fit disabled:bg-[#E1E3EA] disabled:border-[#E1E3EA] disabled:text-white hover:bg-transparent hover:text-[#C72323] border border-[#C72323] text-white text-nowrap bg-[#C72323] rounded-md font-medium leading-[1.25rem] text-sm transition-all">
          Instagram
      ...
      <div className="absolute bottom-8 right-8">
        <BlueButton text={"Save"} disabled={true} />
      </div>
    </div>
  );
};
