  ...
  useEffect(() => {
    if (branchAnimalClasses?.length > 0 && allAnimalClasses?.length > 0) {
      ...
      const existingNames = new Set(
        branchAnimalClasses.map((item) => item.name.toLowerCase())
      );
    ...
    <div className="w-full min-h-full px-[36px] py-4 overflow-y-auto">
      <div className="flex items-start justify-between">
        <div className="text-[#0263E0] text-xs">
          ...
          <span> / </span>
          <p className="underline inline cursor-default">Animal Classes</p>
        </div>
        <div className="flex items-center gap-5">
          ...
          >
            <p className="">{addClasses ? "Close" : "Add"}</p>
          </button>
        </div>
      </div>
      ...
      {branchAnimalClasses && (
        <div className="mt-6 w-full">
          <table className="w-full">
            <thead>
              <tr className="bg-gray-100 text-left text-sm font-medium text-gray-600 border-b border-[#E1E3EA]">
            ...
            <tbody>
              <tr
                className={`${
                  addClasses ? "black" : "hidden"
                } hover:bg-gray-50 hidde text-sm text-gray-700 border-b border-[#E1E3EA]`}
              >
              ...
              {branchAnimalClasses?.map((item, index) => (
                <tr
                  key={index}
                  className="hover:bg-gray-50 text-sm text-gray-700 last:border-b-0 border-b border-[#E1E3EA]"
                >
                  ...
                  <td className="p-2 w-[20%] relative">
                    <button
                      onClick={() => setShowBreedsDropdown(index)}
                      className="text-blue-600 group relative cursor-pointer"
                    >
                      List
                    </button>

                    {showBreedsDropdown === index && (
                      <div className="absolute top-[calc(100%+0.3rem)] z-50 left-1">
                        <BreedsList
                          selectedAnimal={item}
                          setShowBreedsDropdown={setShowBreedsDropdown}
